...
public class MemoController {
    ...
    private final MemoService memmoService;

    @PostMapping("/memos")
    public ResponseEntity<MemoCreateResponse> create(
            @RequestBody MemoCreateRequest request
    ){
        return ResponseEntity.status(HttpStatus.CREATED).body(memmoService.save(request));
    }
    ...
    public ResponseEntity<MemoGetResponse> getOne(
        ...
        return ResponseEntity.status(HttpStatus.OK).body(memmoService.findOne(memoId));
    }
}
